...
const EnrollmentForm = () => {
  ...
  // Auto-fill form with user data when profile/user data is available
  useEffect(() => {
    if (profile && user) {
  ...
  // Watch for real-time profile updates and update form accordingly
  useEffect(() => {
    if (profile && user) {
      const currentValues = form.getValues();
      const updatedValues = {
        ...currentValues,
        first_name: profile?.first_name || currentValues.first_name,
        last_name: profile?.last_name || currentValues.last_name,
        email: user?.email || currentValues.email,
        phone: profile?.contact_number || currentValues.phone,
      };
      
      // Only update if there are actual changes to prevent unnecessary re-renders
      const hasChanges = 
        updatedValues.first_name !== currentValues.first_name ||
        updatedValues.last_name !== currentValues.last_name ||
        updatedValues.email !== currentValues.email ||
        updatedValues.phone !== currentValues.phone;
        
      if (hasChanges) {
        form.setValue('first_name', updatedValues.first_name);
        form.setValue('last_name', updatedValues.last_name);
        form.setValue('email', updatedValues.email);
        form.setValue('phone', updatedValues.phone);
      }
    }
  }, [profile?.first_name, profile?.last_name, profile?.contact_number, user?.email, form]);
  ...
  const onSubmit = async (data: EnrollmentFormData) => {
    ...
    try {
      ...
      // ✅ after inserting into enrollments (status 'pending')
      const { error: profileError } = await supabase
        ...
        .update({
          ...
          application_status: 'enrollment_submitted',   // <<< key change
          updated_at: new Date().toISOString()
        })
        .eq('user_id', user.id);
      
      // Optionally refresh once (no need to loop until 'student')
      await refreshProfile();
      
      toast({
        title: 'Enrollment Submitted Successfully!',
        description:
          'Your enrollment application has been submitted and is under review. Next, please complete payment on the Billing page once it’s opened.',
      });
      ...
      // Force a complete profile refresh with retry mechanism
      let retryCount = 0;
      const maxRetries = 3;
      
      while (retryCount < maxRetries) {
        const updatedProfile = await refreshProfile();
        if (updatedProfile?.application_status === 'student') {
          break;
        }
        retryCount++;
      ...
      toast({
        title: 'Enrollment Submitted Successfully!',
        description: 'Your enrollment application has been submitted and is currently under review by our registrar. You will receive notifications about your application status.',
      });

      // Force navigate and refresh
      navigate('/applicant-dashboard', { replace: true });
    } catch (error) {
                       ...
                       <FormField
                         ...
                         render={({ field }) => (
                           <FormItem>
                             ...
                             <FormControl>
                               <Input placeholder="Enter your city or municipality" {...field} />
                             </FormControl>
                             <FormMessage />
                           </FormItem>
                         )}

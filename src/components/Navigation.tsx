...
import { LogOut, User, Home, BookOpen, Phone, Calendar, Menu, X, ChevronDown } from 'lucide-react';
...
const Navigation = () => {
  ...
  const getNavItems = () => {
    ...
    // Show pre-enrollment only for students and parents
    if (profile?.role === 'student' || profile?.role === 'parent') {
      baseItems.push({ path: '/book-consultation', label: 'Pre-Enrollment', icon: Calendar });
    }
            ...
            {user ? (
              ...
              <div className="flex items-center space-x-3">
                ...
                <Button className="bg-primary hover:bg-primary/90 text-white shadow-md" asChild>
                  <Link to="/auth?mode=signup">Enrollment</Link>
                </Button>
              </div>
            )}
